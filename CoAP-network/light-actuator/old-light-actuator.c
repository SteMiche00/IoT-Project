...
static float room_data[3] = {-99, -99, -99}; // 0=temp, 1=light, 2=humidity
...
static void notify_handler_temp(coap_observee_t *obs, void *notification, coap_notification_flag_t flag) {
  coap_message_t *msg = (coap_message_t *)notification;
  ...
  if(flag == NOTIFICATION_OK && msg != NULL) {
    ...
    if(len > 0) {
      ...
      printf("[ACTUATOR LIGHT] [OBS] Temp: %.3f °C\n", value);
    }
  } else if(flag == OBSERVE_NOT_SUPPORTED) {
    printf("[ACTUATOR LIGHT] [OBS] Observation not supported (temp).\n");
  } 
}

static void notify_handler_humidity(coap_observee_t *obs, void *notification, coap_notification_flag_t flag) {
  coap_message_t *msg = (coap_message_t *)notification;
  ...
  if(flag == NOTIFICATION_OK && msg != NULL) {
    ...
    if(len > 0) {
      ...
      printf("[ACTUATOR LIGHT] [OBS] Humidity: %.3f %%\n", value);
    }
  } else if(flag == OBSERVE_NOT_SUPPORTED) {
    printf("[ACTUATOR LIGHT] [OBS] Observation not supported (humidity).\n");
  } 
}
...
static void discovery_parser(const uint8_t *chunk, int len) {
  ...
  if(sensors_found[0] && sensors_found[1] && sensors_found[2]) {
    ...
    coap_endpoint_parse(sensor_urls[0], strlen(sensor_urls[0]), &server_ep);
    coap_observee_t *obs_temp = coap_observee_add(
        &server_ep,
        resource_paths[0], // "sensors/temp"
        NULL,
        notify_handler_temp,
        NULL);

    if (obs_temp != NULL) {
      printf("[ACTUATOR LIGHT] [OBS] Subscribed to temp sensor!\n");
    } else {
      printf("[ACTUATOR LIGHT] [OBS] Failed to subscribe to temp sensor\n");
    }

    // Subscribe to light sensor
    coap_endpoint_parse(sensor_urls[1], strlen(sensor_urls[1]), &server_ep);
    coap_observee_t *obs_light = coap_observee_add(
        &server_ep,
    ...
    coap_endpoint_parse(sensor_urls[2], strlen(sensor_urls[2]), &server_ep);
    coap_observee_t *obs_humidity = coap_observee_add(
        &server_ep,
        resource_paths[2], // "sensors/humidity"
        NULL,
        notify_handler_humidity,
        NULL);

    if (obs_humidity != NULL) {
      printf("[ACTUATOR LIGHT] [OBS] Subscribed to humidity sensor!\n");
    } else {
      printf("[ACTUATOR LIGHT] [OBS] Failed to subscribe to humidity sensor\n");
    }
